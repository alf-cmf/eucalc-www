...
function createNavTableHeader (tableID) {
  ...
  cellNode.setAttribute("colspan", "3");
    ...
    if (tableID.valueOf() == "overview-3rd-level-nav-table-active-2".valueOf()) {
	txt = document.createTextNode(translate ("Energy"));
    }
    if (tableID.valueOf() == "overview-3rd-level-nav-table-active-3".valueOf()) {
	txt = document.createTextNode(translate ("Emissions"));
    }
...
function appendDataTableRow (name, id, tableNode) {
    ...
    cellNode.setAttribute("style", "white-space: nowrap");
    cellNode.appendChild(document.createTextNode(translate(name)));
    rowNode.appendChild(cellNode);
